...
__kernel void kernel_psinv(__global double *or,
                           __global double *ou,
                           __global double *c,
                           int n1,
                           int n2,
                           int len)
{
  ...
  __global double *r = or + ofs + (zid * n2 + yid) * n1;
  __global double *u = ou + ofs + (zid * n2 + yid) * n1;
...
__kernel void kernel_resid(__global double *or,
                           __global double *ou,
                           __global double *ov,
                           __global double *a,
                           int n1,
                           ...
                           int len)
{
  ...
  __global double *r = or + ofs + (zid * n2 + yid) * n1;
  __global double *u = ou + ofs + (zid * n2 + yid) * n1;
  __global double *v = ov + ofs + (zid * n2 + yid) * n1;

  for (int i = lid + 1; i < len - 1; i += lws) {
    double u11 = (u[i - n1 - 1] + u[i + n1 - 1])
               + (u[i - n2 * n1 - 1] + u[i + n2 * n1 - 1]);

    double u12 = (u[i - n1 + 1] + u[i + n1 + 1])
               + (u[i - n2 * n1 + 1] + u[i + n2 * n1 + 1]);

    double u20 = (u[i - n2 * n1 - n1] + u[i - n2 * n1 + n1])
               + (u[i + n2 * n1 - n1] + u[i + n2 * n1 + n1]);

    double u21 = (u[i - n2 * n1 - n1 - 1] + u[i - n2 * n1 + n1 - 1])
               + (u[i + n2 * n1 - n1 - 1] + u[i + n2 * n1 + n1 - 1]);

    double u22 = (u[i - n2 * n1 - n1 + 1] + u[i - n2 * n1 + n1 + 1])
               + (u[i + n2 * n1 - n1 + 1] + u[i + n2 * n1 + n1 + 1]);
