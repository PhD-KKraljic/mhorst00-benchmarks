...
__kernel void embar(
    ...
    __global double *l_q, __global double *l_sx, __global double *l_sy,
    int k_offset, double an, int k_start, int k_size)
{
  ...
  int l_offset = get_group_id(0) * l_size;
  ...
  for (ii = 0; ii < NK; ii = ii + CHUNK_SIZE) {
    ...
    for (i = 0; i < CHUNK_SIZE; i++) {
      ...
      if (t1 <= 1.0) {
        ...
        l_q[l_offset + l * l_size + l_id] += 1.0;
        sx    = sx + t3;
        sy    = sy + t4;
      }
    }
  ...
  if (l_id == 0) {
    for (i = 0; i < NQ; i++) {
      qqs[get_group_id(0) * NQ + i] = l_q[l_offset + i * l_size + l_id];
    }
    psx[get_group_id(0)] = l_sx[l_offset + 0];
    psy[get_group_id(0)] = l_sy[l_offset + 0];
  }
}
