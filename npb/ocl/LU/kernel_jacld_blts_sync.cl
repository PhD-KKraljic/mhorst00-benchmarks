...
__kernel void jacld_blts_datagen_sync(__global double *m_u, 
...
__kernel void jacld_blts_datacopy_sync(__global double *m_rsd, 
...
void blts_BR_sync(__global double *m_v, 
                  __global double *m_rho_i,
                  __global double *m_u, 
                  __global double *m_qs,
                  __global double *m_a,
                  ...
                  int block_size_k)
{
  ...
  for (step = 0; step < iter; step++) {
    ...
    if (!dummy) {
      a = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_a)[k][j][i];
      b = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_b)[k][j][i];
      c = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_c)[k][j][i];
      d = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_d)[k][j][i];

      for (m = 0; m < 5; m++) {
        temp_v[m] = v[k][j][i][m]
          - omega * (  a[0][m] * v[k-1][j][i][0]
              + a[1][m] * v[k-1][j][i][1]
              + a[2][m] * v[k-1][j][i][2]
              + a[3][m] * v[k-1][j][i][3]
              + a[4][m] * v[k-1][j][i][4] );
      ...
      temp_v[4] = tv[4] / tmat[4][4];

      tv[3] = tv[3] 
        - tmat[3][4] * temp_v[4];
      temp_v[3] = tv[3] / tmat[3][3];

      tv[2] = tv[2]
        - tmat[2][3] * temp_v[3]
        - tmat[2][4] * temp_v[4];
      temp_v[2] = tv[2] / tmat[2][2];

      tv[1] = tv[1]
        - tmat[1][2] * temp_v[2]
        - tmat[1][3] * temp_v[3]
        - tmat[1][4] * temp_v[4];
      temp_v[1] = tv[1] / tmat[1][1];
...
__kernel void jacld_sync(__global double *m_rsd,
                         __global double *m_u,
                         __global double *m_qs,
                         __global double *m_rho_i,
                         __global double *m_a,
                         ...
                         int temp_kst, int temp_kend)
{
  ...
  a = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_a)[k];
  b = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_b)[k];
  c = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_c)[k];
  d = ((__global double (*)[ISIZ2/2*2+1][ISIZ1/2*2+1][5][5])m_d)[k];
...
__kernel void blts_KL_sync(__global double *m_rsd,
                           __global double *m_u,
                           __global double *m_qs,
                           __global double *m_rho_i,
                           __global double *m_a,
