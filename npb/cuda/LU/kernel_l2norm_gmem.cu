...
{ 
  ...
  int step;
  int dummy = 0;
  if (k+work_base >= nz0 - 1 || k+work_base < 1 || k >= work_num_item || j >= jend || i >= iend) dummy = 1;

  if (!split_flag) k += work_base;
  else k += buffer_base;

  double * sum_local =  &(l_sum[l_id * 5]);
  ...
  for (step = l_size/2; step > 0; step = step >> 1) {
    if (l_id < step) {
      for (m = 0; m < 5; m++)
        sum_local[m] += sum_local[step*5+m];
    }
    __syncthreads();
