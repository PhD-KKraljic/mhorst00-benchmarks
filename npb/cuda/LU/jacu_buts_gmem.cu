...
cudaEvent_t     (*ev_k_jbu_datagen_gmem)[2],
                (*ev_k_jbu_KL_gmem)[2];
...
void jacu_buts_init_gmem(int iter, int item_default,
                         int blk_size_k, int blk_size)
{
  int i;

  ev_k_jbu_datagen_gmem =(cudaEvent_t (*)[2])malloc(sizeof(cudaEvent_t)*2*iter);
  ev_k_jbu_KL_gmem =(cudaEvent_t (*)[2])malloc(sizeof(cudaEvent_t)*2*iter);

  for (i = 0; i < iter; i++) {
    cudaEventCreate(&ev_k_jbu_datagen_gmem[i][0]);
    cudaEventCreate(&ev_k_jbu_KL_gmem[i][0]);

    cudaEventCreate(&ev_k_jbu_datagen_gmem[i][1]);
    cudaEventCreate(&ev_k_jbu_KL_gmem[i][1]);
  }
...
void jacu_buts_release_gmem(int iter)
{
  ...
  for (i = 0; i < iter; i++) {
    cudaEventDestroy(ev_k_jbu_datagen_gmem[i][0]);
    cudaEventDestroy(ev_k_jbu_KL_gmem[i][0]);

    cudaEventDestroy(ev_k_jbu_datagen_gmem[i][1]);
    cudaEventDestroy(ev_k_jbu_KL_gmem[i][1]);
  }
                         ...
                         cudaEvent_t *ev_wb_ptr)
{
  ...
  if (split_flag) {
    ...
    CUCHK(cudaEventRecord(ev_k_jbu_datagen_gmem[work_step][0], cmd_q[KERNEL_Q]));

    cuda_ProfilerStartEventRecord("k_jbu_datagen_gmem",  cmd_q[KERNEL_Q]);
    k_jbu_datagen_gmem<<<numBlocks, numThreads, 0, cmd_q[KERNEL_Q]>>>
      (
       m_u[buf_idx],
       m_qs[buf_idx],
       m_rho_i[buf_idx],
    ...
    cuda_ProfilerEndEventRecord("k_jbu_datagen_gmem",  cmd_q[KERNEL_Q]);

    CUCHK(cudaEventRecord(ev_k_jbu_datagen_gmem[work_step][1], cmd_q[KERNEL_Q]));
                                  ...
                                  int temp_kend)
{
  ...
  CUCHK(cudaEventRecord(ev_k_jbu_KL_gmem[work_step][0], cmd_q[KERNEL_Q]));
  ...
  for (k = (temp_kend-temp_kst-1)+(iend-ist-1)+(jend-jst-1); k >= 0; k--) {
    ...
    cuda_ProfilerStartEventRecord("k_jbu_KL_gmem",  cmd_q[KERNEL_Q]);
    k_jbu_KL_gmem<<< numBlocks, numThreads, 0, cmd_q[KERNEL_Q]>>>
      (
    ...
    CUCHK(cudaGetLastError());
    cuda_ProfilerEndEventRecord("k_jbu_KL_gmem",  cmd_q[KERNEL_Q]);
  }

  CUCHK(cudaEventRecord(ev_k_jbu_KL_gmem[work_step][1], cmd_q[KERNEL_Q]));
