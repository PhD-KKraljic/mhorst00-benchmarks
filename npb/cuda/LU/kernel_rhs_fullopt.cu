...
{
  extern __shared__ double l_flux[];
  double *m_l_u = &l_flux[(blockDim.x + 2) * 5];
...
void k_rhsx3_fullopt(double *m_u, double *m_rsd,
                     ...
                     int split_flag)
{
  extern __shared__ double m_l_u[];
...
__launch_bounds__(min(ISIZ1-1 - 1, MAX_THREAD_DIM_0))
__global__ 
