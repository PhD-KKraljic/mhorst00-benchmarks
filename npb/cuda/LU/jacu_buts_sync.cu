...
cudaEvent_t       (*ev_k_jbu_datagen_sync)[2],
                  (*ev_k_jacu_sync)[2],
                  (*ev_k_buts_BR_sync)[2],
                  (*ev_k_buts_KL_sync)[2];
...
void jacu_buts_init_sync(int iter, int item_default,
                         int blk_size_k, int blk_size)
{
  int i, tmp_work_end,
      tmp_work_num_item,
      tmp_work_base,
      tmp_kend, tmp_kst, kst, kend;
  double start_t, end_t, KL_time, BR_time;
  size_t jbu_work_num_item;

  ev_k_jbu_datagen_sync = (cudaEvent_t (*)[2])malloc(sizeof(cudaEvent_t)*2*iter);
  ev_k_jacu_sync = (cudaEvent_t (*)[2])malloc(sizeof(cudaEvent_t)*2*iter);
  ev_k_buts_BR_sync = (cudaEvent_t (*)[2])malloc(sizeof(cudaEvent_t)*2*iter);
  ev_k_buts_KL_sync = (cudaEvent_t (*)[2])malloc(sizeof(cudaEvent_t)*2*iter);

  for (i = 0; i < iter; i++) {
    cudaEventCreate(&ev_k_jbu_datagen_sync[i][0]);
    cudaEventCreate(&ev_k_jacu_sync[i][0]);
    cudaEventCreate(&ev_k_buts_BR_sync[i][0]);
    cudaEventCreate(&ev_k_buts_KL_sync[i][0]);

    cudaEventCreate(&ev_k_jbu_datagen_sync[i][1]);
    cudaEventCreate(&ev_k_jacu_sync[i][1]);
    cudaEventCreate(&ev_k_buts_BR_sync[i][1]);
    cudaEventCreate(&ev_k_buts_KL_sync[i][1]);
  }
...
void jacu_buts_release_sync(int iter)
{
  ...
  for (i = 0; i < iter; i++) {
    cudaEventDestroy(ev_k_jbu_datagen_sync[i][0]);
    cudaEventDestroy(ev_k_jacu_sync[i][0]);
    cudaEventDestroy(ev_k_buts_BR_sync[i][0]);
    cudaEventDestroy(ev_k_buts_KL_sync[i][0]);

    cudaEventDestroy(ev_k_jbu_datagen_sync[i][1]);
    cudaEventDestroy(ev_k_jacu_sync[i][1]);
    cudaEventDestroy(ev_k_buts_BR_sync[i][1]);
    cudaEventDestroy(ev_k_buts_KL_sync[i][1]);
  }
                         ...
                         cudaEvent_t *ev_wb_ptr)
{
  ...
  if (split_flag) {
    ...
    CUCHK(cudaEventRecord(ev_k_jbu_datagen_sync[work_step][0], cmd_q[KERNEL_Q]));

    cuda_ProfilerStartEventRecord("k_jbu_datagen_sync",  cmd_q[KERNEL_Q]);
    k_jbu_datagen_sync<<<numBlocks, numThreads, 0, cmd_q[KERNEL_Q]>>>
      (
       m_u[buf_idx],
       m_qs[buf_idx],
       m_rho_i[buf_idx],
    ...
    cuda_ProfilerEndEventRecord("k_jbu_datagen_sync",  cmd_q[KERNEL_Q]);
    
    CUCHK(cudaEventRecord(ev_k_jbu_datagen_sync[work_step][1], cmd_q[KERNEL_Q]));
                      ...
                      int temp_kend)
{
  ...
  CUCHK(cudaEventRecord(ev_k_jacu_sync[work_step][0], cmd_q[KERNEL_Q]));

  cuda_ProfilerStartEventRecord("k_jacu_sync",  cmd_q[KERNEL_Q]);
  k_jacu_sync<<< numBlocks, numThreads, 0, cmd_q[KERNEL_Q]>>>
    (
  ...
  CUCHK(cudaGetLastError());
  cuda_ProfilerEndEventRecord("k_jacu_sync",  cmd_q[KERNEL_Q]);

  CUCHK(cudaEventRecord(ev_k_jacu_sync[work_step][1], cmd_q[KERNEL_Q]));
                         ...
                         int temp_kend)
{
  ...
  int iter, diagonals, depth, num_wg;
  int buf_idx = (work_step%2)*buffering_flag;
  ...
  CUCHK(cudaEventRecord(ev_k_buts_BR_sync[work_step][0], cmd_q[KERNEL_Q]));
  ...
  for (iter = 0; iter < num_block_k + num_block_j + num_block_i - 2; iter++) {
    ...
    cuda_ProfilerStartEventRecord("k_buts_BR_sync",  cmd_q[KERNEL_Q]);
    k_buts_BR_sync<<< numBlocks, numThreads, 0, cmd_q[KERNEL_Q]>>>
      (m_rsd[buf_idx],
       m_rho_i[buf_idx],
       m_u[buf_idx],
       m_qs[buf_idx],
    ...
    CUCHK(cudaGetLastError());
    cuda_ProfilerEndEventRecord("k_buts_BR_sync",  cmd_q[KERNEL_Q]);
  ...
  CUCHK(cudaEventRecord(ev_k_buts_BR_sync[work_step][1], cmd_q[KERNEL_Q]));
}
                         ...
                         int temp_kend)
{
  ...
  CUCHK(cudaEventRecord(ev_k_buts_KL_sync[work_step][0], cmd_q[KERNEL_Q]));
  ...
  for (k = (temp_kend-temp_kst-1)+(iend-ist-1)+(jend-jst-1); k >= 0; k--) {
    ...
    cuda_ProfilerStartEventRecord("k_buts_KL_sync",  cmd_q[KERNEL_Q]);
    k_buts_KL_sync<<< numBlocks, numThreads, 0, cmd_q[KERNEL_Q]>>>
      (
        m_rsd[buf_idx],
        m_u[buf_idx],
        m_qs[buf_idx],
    ...
    CUCHK(cudaGetLastError());
    cuda_ProfilerEndEventRecord("k_buts_KL_sync",  cmd_q[KERNEL_Q]);

  }

  CUCHK(cudaEventRecord(ev_k_buts_KL_sync[work_step][1], cmd_q[KERNEL_Q]));
}
