                        ...
                        int split_flag)
{
  ...
  double (* g_lhs)[PROBLEM_SIZE-1][PROBLEM_SIZE+1][3][5][5]
    = (double (*)[PROBLEM_SIZE-1][PROBLEM_SIZE+1][3][5][5])m_lhs;

  double (* lhs)[3][5][5] = g_lhs[k][j];
  ...
  double (* fjac)[5][5] = g_fjac[k][j];
  double (* njac)[5][5] = g_njac[k][j];
                        ...
                        int split_flag)
{
  ...
  double (* fjac)[5][5] = g_fjac[k][i];
  double (* njac)[5][5] = g_njac[k][i];
...
#define rhs(a, b, c, d)    m_rhs[(((a) * WORK_NUM_ITEM_DEFAULT_J + (b)) * (IMAXP+1) + (c)) * 5 + (d)]
  ...
  double (* fjac)[5][5] = g_fjac[j][i];
  double (* njac)[5][5] = g_njac[j][i];
  ...
  // multiply rhs(0) by b_inverse(0) and copy to rhs
  //---------------------------------------------------------------------
  binvcrhs( lhs[0][BB], lhs[0][CC], &rhs(0, j, i, 0) );
  ...
  for (k = 1; k <= ksize-1; k++) {
    ...
    //-------------------------------------------------------------------
    matvec_sub(lhs[k][AA], &rhs(k-1, j, i, 0), &rhs(k, j, i, 0));
    ...
    //-------------------------------------------------------------------
    binvcrhs( lhs[k][BB], lhs[k][CC], &rhs(k, j, i, 0) );
  }
  ...
  //---------------------------------------------------------------------
  matvec_sub(lhs[ksize][AA], &rhs(ksize-1, j, i, 0), &rhs(ksize, j, i, 0));
  ...
  //---------------------------------------------------------------------
  binvrhs( lhs[ksize][BB], &rhs(ksize, j, i, 0) );
