...
/* write cuda device info for compile */
...
void write_cuda_info(FILE *fp)
{
  ...
  if (err != cudaSuccess) {
    ...
    maxThreadsDim[0] = prop.maxThreadsDim[0];
    maxThreadsDim[1] = prop.maxThreadsDim[1];
    maxThreadsDim[2] = prop.maxThreadsDim[2];
  }

  fprintf(fp, "\n/* CUDA device info */\n");
  fprintf(fp, "#define MAX_THREAD_BLOCK_SIZE  %d\n", maxThreadsPerBlock);
  fprintf(fp, "#define MAX_THREAD_DIM_0       %d\n", maxThreadsDim[0]);
  fprintf(fp, "#define MAX_THREAD_DIM_1       %d\n", maxThreadsDim[1]);
  fprintf(fp, "#define MAX_THREAD_DIM_2       %d\n", maxThreadsDim[2]);
