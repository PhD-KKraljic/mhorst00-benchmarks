...
__host__ __device__ double randlc( double *x, double a )
{
    /*
...
__host__ __device__ void vranlc( int n, double *x, double a, double y[] )
{
    /*
...
__global__ void embar_base(
        double *qqs, double *psx, double *psy,
        double *l_q, double *l_sx, double *l_sy,
        int k_offset, double an, int k_start, int k_size)
{
    ...
    int l_offset = blockIdx.x * l_size;
    ...
    for (i = 0; i < NQ; i++) {
        l_q[l_offset + i * l_size + l_id] = 0.0;
    }
    ...
    l_sx[l_offset + l_id] = sx;
    l_sy[l_offset + l_id] = sy;
    __syncthreads();

    for (i = 0; i < NQ; i++) {
        for (int p = blockDim.x / 2; p >= 1; p = p >> 1) {
            if (l_id < p) {
                l_q[l_offset + i * l_size + l_id] += l_q[l_offset + i * l_size + l_id + p];
            }
            __syncthreads();
        }
    }
    for (int p = blockDim.x / 2; p >= 1; p = p >> 1) {
        if (l_id < p) {
            l_sx[l_offset + l_id] += l_sx[l_offset + l_id + p];
            l_sy[l_offset + l_id] += l_sy[l_offset + l_id + p];
        }
        __syncthreads();
    }
    if (l_id == 0) {
        for (i = 0; i < NQ; i++) {
            qqs[blockIdx.x * NQ + i] = l_q[l_offset + i * l_size + l_id];
        }
        psx[blockIdx.x] = l_sx[l_offset + 0];
        psy[blockIdx.x] = l_sy[l_offset + 0];
    }
}

__global__ void embar_opt(
        double *qqs, double *psx, double *psy,
        int l_q_size, int l_sx_size, int l_sy_size,
        int k_offset, double an, int k_start, int k_size)
{
    extern __shared__ double dynamic_shared[];
    double *l_q = dynamic_shared;
    double *l_sx = &l_q[l_q_size];
    double *l_sy = &l_sx[l_sx_size];
    int g_id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (l_id == 0) {
        for (i = 0; i < NQ; i++) {
            qqs[blockIdx.x * NQ + i] = l_q[i * l_size + l_id];
        }
        psx[blockIdx.x] = l_sx[0];
        psy[blockIdx.x] = l_sy[0];
    }
}
#undef CHUNK_SIZE
#endif
